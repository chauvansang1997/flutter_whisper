...
JNIEXPORT jstring JNICALL
Java_com_whispercpp_whisper_WhisperLib_00024Companion_detectLanguages(JNIEnv *env, jobject thiz,
                                                                      jlong context_ptr,
                                                                      jint num_threads,
    ...
    LOGI("About to run detect language");

    const char* language = whisper_full_detect_language(context, params, audio_data_arr, audio_data_length);
    LOGI("auto-detect language %s\n", language);

    (*env)->ReleaseFloatArrayElements(env, audio_data, audio_data_arr, JNI_ABORT);

    jstring string = (*env)->NewStringUTF(env, language);

    return string;
}

JNIEXPORT void JNICALL
Java_com_whispercpp_whisper_WhisperLib_00024Companion_fullTranscribeWithLanguage(JNIEnv *env,
                                                                                 jobject thiz,
                                                                                 jlong context_ptr,
                                                                                 jstring language,
                                                                                 jint num_threads,
    ...
    jfloat *audio_data_arr = (*env)->GetFloatArrayElements(env, audio_data, NULL);
    const jsize audio_data_length = (*env)->GetArrayLength(env, audio_data);
    const char *ctrLanguage = (*env)->GetStringUTFChars(env, language, NULL);
    ...
    LOGI("About to run whisper_full with %s", params.language);
    ...
    (*env)->ReleaseStringUTFChars(env, language, ctrLanguage);
    (*env)->ReleaseFloatArrayElements(env, audio_data, audio_data_arr, JNI_ABORT);
}